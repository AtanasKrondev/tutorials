  ...
  <form @submit.prevent="submit">
    <fieldset>
      ...
      <template v-if="$v.username.$error">
        ...
        <p
          v-else-if="!$v.username.username"
          class="error"
        >Username should be two names starting with capital!</p>
      </template>
      <!-- end if error -->
      ...
      <template v-if="$v.email.$error">
        ...
        <p v-else-if="!$v.email.email" class="error">Email format is invalid!</p>
      </template>
      <!-- end if error -->

      <p class="field field-icon">
        <label for="tel-prefix">
          <span>
            <i class="fas fa-phone"></i>
          </span>
        </label>
        <select name="tel-prefix" id="tel-prefix" class="tel">
      ...
      <template v-if="$v.tel.$error">
        ...
        <p v-else-if="!$v.tel.tel" class="error">Phone should be 9 digits!</p>
      </template>
      <!-- end if error -->
      ...
      <template v-if="$v.password.$error">
        ...
        <p
          ...
          class="error"
        >Password must be between 3 and 16 symbols!</p>
        <p
          v-else-if="!$v.password.alphaNum"
          class="error"
        >Password must contain only digits and letters!</p>
      </template>
      ...
      <p class="field field-icon">
        ...
        <input
          ...
          :class="{'error':$v.rePassword.$error}"
        />
      </p>
...
const username = helpers.regex("username", /^[A-Z][a-z]+ [A-Z][a-z]+$/);
const tel = helpers.regex("tel", /^[0-9]{9}$/);
